"""Thesis Activity: Quiz and Feedback Generators (Generic prompts - to be customized)."""
...
class QuizGeneratorThesis:
    """Quiz generator for Thesis annotations with language detection."""

    def __init__(self, model_name="mistralai/mistral-7b-instruct"):
        """Initialize the Thesis quiz generator."""
    ...
    def _get_language_instructions(self, detected_lang: str) -> dict:
        """Get language-specific instructions for Thesis quiz generation."""
        if detected_lang == 'en':
            return {
                "task_instruction": "Create a comprehension quiz in ENGLISH that assesses understanding of the thesis and arguments in the text.",
...
- Everything else (questions, options) must be in ENGLISH."""
            ...
            return {
                "task_instruction": "Crea un quiz di comprensione in italiano che valuti la comprensione della tesi e degli argomenti nel testo.",
...
- Tutto il resto deve essere nella lingua del testo."""
    ...
    def generate_quiz(self, text: str, annotations: Dict[str, List[str]]) -> str:
        """Generate a Thesis quiz (GENERIC - to be customized)."""
        if not self.openrouter_client.is_available():
...
Ogni domanda deve essere basata sulle annotazioni fornite e testare la comprensione della struttura argomentativa.
...
class FeedbackGeneratorThesis:
    """Feedback generator for Thesis activity (GENERIC - to be customized)."""

    def __init__(self, model_name="mistralai/mistral-7b-instruct"):
        """Initialize the Thesis feedback generator."""
        self.model_name = model_name
        self.openrouter_client = OpenRouterClient()
    ...
    def _generate_oe_feedback(self, question: str, correct_answer: str, student_answer: str, 
        ...
        """Generic open-ended feedback for Thesis."""
        ...
        prompt = f"""Sei un tutor educativo. Fornisci feedback sulla risposta dello studente basandoti sul testo e le annotazioni (Thesis).
...
FEEDBACK:"""
    ...
    def _generate_mc_feedback(self, question: str, correct_answer: str, student_answer: str,
        ...
        """Generic multiple choice feedback for Thesis."""
        ...
        prompt = f"""Feedback per scelta multipla (Thesis).
